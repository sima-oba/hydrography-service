...
@click.option('--timeout', default=300, type=click.INT)
def start_gunicorn_server(host: str, port: int, workers: int, timeout: int):
    from application import rest
    rest.start_gunicorn_server(Config, host, port, workers, timeout)
